...
const nodemailer = require('nodemailer'); // Use Nodemailer for emails
...
// Initialize Nodemailer with your Gmail App Password
...
app.post('/api/requests', async (req, res) => {
    try {
        ...
        const nearbyDonors = await Donor.find({
            ...
            location: {
                $near: { $geometry: { type: 'Point', coordinates: [longitude, latitude] }, $maxDistance: 20000 }
            }
        });
        ...
        console.error("CRASH in /api/requests:", error);
...
app.get('/api/confirm/:requestId/:donorId', async (req, res) => {
    try {
        ...
        const mailToDonor = {
            ...
            html: `<p>Thank you, ${donor.name}!</p><p>You have been matched. Please contact the person in need immediately:</p><ul><li>Name: ${updatedRequest.requesterName}</li><li>Phone: ${updatedRequest.requesterPhone}</li></ul>`
        };
        ...
        const mailToRequester = {
            ...
            html: `<p>Hello ${updatedRequest.requesterName},</p><p>Great news! A donor has confirmed they are available and will be contacting you shortly.</p><ul><li>Donor's Name: ${donor.name}</li></ul>`
        };
        await transporter.sendMail(mailToRequester);

        res.send('<h1>Confirmed!</h1><p>You are a match! Details have been sent via email.</p>');
    } catch (error) {
